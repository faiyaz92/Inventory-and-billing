...
class FirestorePathProviderImpl implements IFirestorePathProvider {
  ...
  static const String tasksCollection = 'tasks';  // ✅ New Task Collection Constant
  static const String users = 'users';  // ✅ New Task Collection Constant

  FirestorePathProviderImpl(this._firestore);

  @override
  DocumentReference get basePath => _firestore.collection(rootPath).doc(companyDirectory);
  ...
  DocumentReference getTenantUserRef(String companyId, String userId) {
    return getTenantCompanyRef(companyId).collection(usersCollection).doc(userId);
  }
  ...
  CollectionReference getCustomerCompanyRef(String companyId) {
    return getTenantCompanyRef(companyId).collection(companiesCollection); // 🔹 Actual collection name remains `companies`
  }

  @override
  DocumentReference getSingleCustomerCompanyRef(String companyId, String customerCompanyId) {
  ...
  DocumentReference getAccountLedgerRef(String companyId, String ledgerId) {
    return getTenantCompanyRef(companyId).collection('accountLedgers').doc(ledgerId);
  }
