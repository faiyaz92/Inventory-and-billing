...
class TaxiSettingsRepositoryImpl implements ITaxiSettingsRepository {
  ...
  Future<TaxiSettings> getSettings(String companyId) async {
    ...
    final settingsSnapshot = await _pathProvider.getTaxiBookingSettingsRef(companyId).get();
    final settingsData = settingsSnapshot.exists ? settingsSnapshot.data() as Map<String, dynamic> : {};

    // Fetch subcollections
    final taxiTypesSnapshot = await _pathProvider.getTaxiTypesCollectionRef(companyId).get();
    final tripTypesSnapshot = await _pathProvider.getTripTypesCollectionRef(companyId).get();
    final serviceTypesSnapshot = await _pathProvider.getServiceTypesCollectionRef(companyId).get();
  ...
  Future<void> updateSettings(String companyId, TaxiSettings settings) async {
    final dto = TaxiSettingsDto(
      ...
      whatsappNotificationFareThreshold: settings.whatsappNotificationFareThreshold,
      updatedAt: DateTime.now(),
      updatedBy: settings.updatedBy,
      taxiTypes: settings.taxiTypes.map((type) => TaxiTypeDto.fromModel(type)).toList(),
      tripTypes: settings.tripTypes.map((type) => TripTypeDto.fromModel(type)).toList(),
      serviceTypes: settings.serviceTypes.map((type) => ServiceTypeDto.fromModel(type)).toList(),
      tripStatuses: settings.tripStatuses.map((status) => TripStatusDto.fromModel(status)).toList(),
    );

    // Update settings document
    await _pathProvider.getTaxiBookingSettingsRef(companyId).set(dto.toFirestore());
    ...
    final serviceTypesRef = _pathProvider.getServiceTypesCollectionRef(companyId);
  ...
  Future<void> addTaxiType(String companyId, TaxiType type) async {
    ...
    await _pathProvider.getTaxiTypesCollectionRef(companyId).doc(type.name).set(dto.toFirestore());
  }

  @override
  Future<void> deleteTaxiType(String companyId, String typeId) async {
    await _pathProvider.getTaxiTypesCollectionRef(companyId).doc(typeId).delete();
  }

  @override
  Future<void> addTripType(String companyId, TripType type) async {
    final dto = TripTypeDto.fromModel(type);
    await _pathProvider.getTripTypesCollectionRef(companyId).doc(type.name).set(dto.toFirestore());
  }

  @override
  Future<void> deleteTripType(String companyId, String typeId) async {
    await _pathProvider.getTripTypesCollectionRef(companyId).doc(typeId).delete();
  }

  @override
  Future<void> addServiceType(String companyId, ServiceType type) async {
    final dto = ServiceTypeDto.fromModel(type);
    await _pathProvider.getServiceTypesCollectionRef(companyId).doc(type.name).set(dto.toFirestore());
  }

  @override
  Future<void> deleteServiceType(String companyId, String typeId) async {
    await _pathProvider.getServiceTypesCollectionRef(companyId).doc(typeId).delete();
  }

  @override
  Future<void> addTripStatus(String companyId, TripStatus status) async {
    final dto = TripStatusDto.fromModel(status);
    await _pathProvider.getTripStatusesCollectionRef(companyId).doc(status.name).set(dto.toFirestore());
  }

  @override
  Future<void> deleteTripStatus(String companyId, String statusId) async {
    await _pathProvider.getTripStatusesCollectionRef(companyId).doc(statusId).delete();
  }
